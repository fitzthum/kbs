...
impl AttestationTokenBroker for SimpleAttestationTokenBroker {
    ...
    fn issue(
		...
		if !policy_results.contains_key(ALLOW_KEY) {
			bail!("Policy results must contain allow claim");
		}
		
		let policy_result = policy_results.get(ALLOW_KEY).unwrap().as_string()?;

		let evaluation_report = json!({
			"policy-id": policy_id,
        ...
        let token_claims = json!({
            ...
            "evaluation-reports": evaluation_report,
